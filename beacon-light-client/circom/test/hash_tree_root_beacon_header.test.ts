...
import * as update from "../../../vendor/eth2-light-client-updates/mainnet/updates/00291.json";
...
describe("Hash tree root beacon header test", () => {
  it("Test1", async () => {
    ...
    let input = {
      ...
      parent_root: BigInt("0x" + bytesToHex(block_header.parentRoot as Uint8Array)).toString(2).padStart(256, '0').split(''),
      state_root: BigInt("0x" + bytesToHex(block_header.stateRoot as Uint8Array)).toString(2).padStart(256, '0').split(''),
      body_root: BigInt("0x" + bytesToHex(block_header.bodyRoot as Uint8Array)).toString(2).padStart(256, '0').split('')
    };

    const hash = ssz.phase0.BeaconBlockHeader.hashTreeRoot(block_header);

    const circuit = await fastestTester("./scripts/hash_tree_root_beacon_header/hash_tree_root_beacon_header.circom");
    ...
    expect(BigInt("0x" + bytesToHex(hash)).toString(2).padStart(256, '0')).to.be.eq(a);
  });
});
